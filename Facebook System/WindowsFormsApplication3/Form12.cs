...
namespace WindowsFormsApplication3
{
    public partial class Form12 : Form
    {
        ...
        private void comment1_Click(object sender, EventArgs e)
        {
            ...
            cmd.Parameters.AddWithValue("@name", "Community" );
            cmd.ExecuteNonQuery();
        ...
        {
            string FF = @"SELECT likes_group From Post WHERE email=@username and Name_group='Community'";
            string FFFF = null;
            cnn.Open();
        ...
        {
            cnn.Open();
            SqlCommand c = new SqlCommand("Select  MAX(G_ID) From Groupfacebook Where (email = '" + Globals.emails + "' and GroupName ='Community' )", cnn);
            object result = c.ExecuteScalar();
            result = (result == DBNull.Value) ? null : result;
            int d = Convert.ToInt32(result);
            cmd = new SqlCommand("Select photo From Groupfacebook Where (email = '" + Globals.emails + "' and GroupName ='Community'and photo is not NULL and G_ID = '" + d + "')", cnn); SqlDataReader sdr = cmd.ExecuteReader();
            while (sdr.Read())
            {
        ...
        private void textBox2_VisibleChanged(object sender, EventArgs e)
        {
            ...
            result = (result == DBNull.Value) ? null : result;
            int d = Convert.ToInt32(result);
            cmd = new SqlCommand("Select  post From Groupfacebook Where (email = '" + Globals.emails + "' and GroupName ='Community' and post is not NULL and G_ID = '" + d + "')", cnn); string t = (string)cmd.ExecuteScalar();
            if (t == "NULL")
            {
