...
namespace WindowsFormsApplication3
{
    public partial class Form13 : Form
    {
        ...
        private void comment1_Click(object sender, EventArgs e)
        {
            ...
            cmd.Parameters.AddWithValue("@name", "Ultras FCIS" );
            cmd.ExecuteNonQuery();
        ...
        {
            string FF = @"SELECT likes_group From Post WHERE email=@username and Name_group='Ultras FCIS'";
            string FFFF = null;
            cnn.Open();
        ...
        {
            cnn.Open();
            SqlCommand c = new SqlCommand("Select  MAX(G_ID) From Groupfacebook Where (email = '" + Globals.emails + "' and GroupName ='Ultras FCIS')", cnn);
            object result = c.ExecuteScalar();
            result = (result == DBNull.Value) ? null : result;
            int d = Convert.ToInt32(result);
            cmd = new SqlCommand("Select photo From Groupfacebook Where (email = '" + Globals.emails + "' and GroupName ='Ultras FCIS'and photo is not NULL and G_ID = '" + d + "')", cnn); SqlDataReader sdr = cmd.ExecuteReader();
            while (sdr.Read())
            {
        ...
        private void textBox2_VisibleChanged(object sender, EventArgs e)
        {
            ...
            object result = c.ExecuteScalar();
            result = (result == DBNull.Value) ? null : result;
            int d = Convert.ToInt32(result); cmd = new SqlCommand("Select  post From Groupfacebook Where (email = '" + Globals.emails + "' and GroupName ='Ultras FCIS' and post is not NULL and G_ID = '" + d + "')", cnn); string t = (string)cmd.ExecuteScalar();
            if (t == "NULL")
            {
